...
const executeVersionProbe = (executablePath, arguments_, timeoutMs, signal) => new Promise((resolve, reject) => {
...
export class NodeFfmpegMediaProbeAdapter {
    #executablePath;
    constructor(executablePath = "/usr/bin/ffmpeg") {
    ...
    async probe(input) {
        ...
        try {
            const output = await executeVersionProbe(executablePath, ["-version"], input.timeoutMs, input.signal);
            const firstLine = output.split(/\r?\n/u)[0]?.trim() ?? "";
            if (!firstLine.toLowerCase().startsWith("ffmpeg version ") ||
...
export class NodeYtDlpMediaProbeAdapter {
    #executablePath;
    constructor(executablePath = "/usr/bin/yt-dlp") {
        this.#executablePath = executablePath;
    }
    async probe(input) {
        const executablePath = this.#executablePath.trim();
