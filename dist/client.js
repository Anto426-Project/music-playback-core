...
export class MusicPlaybackClient {
    ...
    async queue(guildIdInput, signal) {
        const runtime = this.#readyRuntime();
        const guildId = boundedIdentity(guildIdInput, "Guild id");
        ...
        const snapshot = await this.#executeTrackedOperation(runtime, guildId, (activeRuntime, operationSignal) => activeRuntime.queue(guildId, operationSignal), this.#options.sessionReadTimeoutMs, () => new MusicPlaybackError("MUSIC.MEDIA_ENGINE_UNAVAILABLE", "The music queue read timed out.", true), signal);
        if (snapshot === null)
            return null;
