...
- The `./discord-player` entrypoint owns Discord Player v7, its extractors,
  YouTubei, bounded yt-dlp streaming and the private Discord.js extension.
...
- `NodeDiscordPlayerExtension` registers with `discord-bot-core`'s opaque,
  generation-aware extension host. Products never receive a provider client;
  no Discord.js type is emitted in package declarations.
...
Antobot composes this package together with `discord-bot-core`. UniBot does not
depend on Antobot or this package: it composes only `discord-bot-core` and its
University-owned features. Sharing a technical package never creates a runtime
call or an ownership relationship between the two products.
