...
import type {
  ...
  MediaProbePort,
  MediaResolverPort,
  MusicPlaybackClientHealth,
  MusicPlaybackClientLifecyclePort,
  MusicProviderHealth
} from "./ports.js";
import {
  isMusicProviderKey,
  normalizeMusicProviderPolicies,
  selectMusicProvider
...
export const DEFAULT_MUSIC_PLAYBACK_CLIENT_OPTIONS:
MusicPlaybackClientOptions = Object.freeze({
...
const abortError = (
  ...
  signal: AbortSignal
): unknown => signal.reason ?? fallback;

const executeBounded = async <T>(
  operation: (signal: AbortSignal) => Promise<T>,
  timeoutMs: number,
  timeoutError: () => MusicPlaybackError,
  ...
  if (callerSignal?.aborted === true) {
    throw abortError(
      new MusicPlaybackError(
        ...
        "The music operation was cancelled.",
        true
      ),
      callerSignal
    );
...
const uniqueProviders = (
  providers: readonly MusicProviderKey[],
  label: string
): readonly MusicProviderKey[] => {
  if (
...
const normalizedDnsHostname = (value: unknown): string => {
...
const validateOptions = (
  options: MusicPlaybackClientOptions
): MusicPlaybackClientOptions => {
  const enabledProviders = uniqueProviders(
    options.enabledProviders,
    "Enabled providers"
  );
  const bridgeProviderOrder = uniqueProviders(
    options.bridgeProviderOrder,
    "Bridge provider order"
  );
  if (
    bridgeProviderOrder.some(
      (provider) =>
        !(MUSIC_STREAM_PROVIDER_KEYS as readonly MusicProviderKey[]).includes(
          provider
        )
    )
  ) {
  ...
  const enabledBridgeProviderOrder = bridgeProviderOrder.filter((provider) =>
    enabledProviders.includes(provider)
  );
  if (
    enabledProviders.some(
      (provider) => provider === "spotify" || provider === "apple_music"
    ) &&
    enabledBridgeProviderOrder.length === 0
  ) {
    throw new TypeError(
      "Metadata sources require at least one enabled streaming bridge."
    );
  }
  if (
  ...
  const maximumConcurrentOperations = boundedInteger(
    options.maximumConcurrentOperations,
    1,
    1_024,
    "maximumConcurrentOperations"
  );
  const maximumConcurrentOperationsPerGuild = boundedInteger(
    options.maximumConcurrentOperationsPerGuild,
    1,
    64,
    "maximumConcurrentOperationsPerGuild"
  );
  if (
    maximumConcurrentOperationsPerGuild > maximumConcurrentOperations
  ) {
    throw new TypeError(
      "The per-guild operation limit cannot exceed the global limit."
    );
  }
  return Object.freeze({
    ...
    mediaProbeTimeoutMs: boundedInteger(
      options.mediaProbeTimeoutMs,
      100,
      30_000,
      "mediaProbeTimeoutMs"
    ),
    providerRegistrationTimeoutMs: boundedInteger(
      options.providerRegistrationTimeoutMs,
      100,
      120_000,
      "providerRegistrationTimeoutMs"
    ),
    mediaResolutionTimeoutMs: boundedInteger(
      options.mediaResolutionTimeoutMs,
      100,
      120_000,
    ...
    sessionReadTimeoutMs: boundedInteger(
      options.sessionReadTimeoutMs,
      100,
      30_000,
    ...
    maximumConcurrentOperations,
    maximumConcurrentOperationsPerGuild,
    maximumActiveSessions: boundedInteger(
      options.maximumActiveSessions,
      1,
      1_000,
      "maximumActiveSessions"
    ),
    maximumQueryLength: boundedInteger(
      options.maximumQueryLength,
      1,
      2_000,
      "maximumQueryLength"
    ),
    maximumSearchResults: boundedInteger(
      options.maximumSearchResults,
      1,
      100,
      "maximumSearchResults"
    ),
    maximumResolvedReferences: boundedInteger(
      options.maximumResolvedReferences,
      1,
      50_000,
      "maximumResolvedReferences"
    ),
    maximumTrackDurationMs: boundedInteger(
      options.maximumTrackDurationMs,
      1,
      24 * 60 * 60 * 1_000,
      "maximumTrackDurationMs"
    )
  });
};

const stoppedHealth = (): MusicPlaybackClientHealth =>
  Object.freeze({
    state: "stopped",
    mediaEngine: null,
...
const healthFor = (
  ...
  state: MusicProviderHealth["state"],
  failureCode: string | null
): MusicProviderHealth => Object.freeze({ provider, state, failureCode });

const normalizeProbeResult = (
  value: MediaEngineProbeResult
): MediaEngineProbeResult => {
...
const boundedReference = (value: unknown, label: string): string => {
  if (
    ...
    throw new MusicPlaybackError(
      ...
      `The music runtime returned an invalid ${label}.`,
      true
    );
  }
  return value;
...
const isRepeatMode = (value: unknown): value is MusicRepeatMode =>
...
const normalizeControl = (
  ...
  if (control === null || typeof control !== "object") {
    throw new MusicPlaybackError(
      "MUSIC.CONTROL_INVALID",
      "The playback control is invalid.",
      false
    );
  }
  switch (control.kind) {
    ...
    case "set_volume":
      if (
        !Number.isSafeInteger(control.volume) ||
        control.volume < 0 ||
        control.volume > 100
      ) {
        throw new MusicPlaybackError(
    ...
    default:
      throw new MusicPlaybackError(
        "MUSIC.CONTROL_INVALID",
        "The playback control kind is invalid.",
        false
      );
  }
};

type MusicPlaybackClientState =
  | "stopped"
  | "starting"
  | "running"
  | "stopping";

const clientNotReadyError = (message: string): MusicPlaybackError =>
  new MusicPlaybackError("MUSIC.CLIENT_NOT_STARTED", message, true);
...
export class MusicPlaybackClient
  implements
    ...
    AudioPlaybackPort
{
  readonly #factory: MusicPlaybackRuntimeFactory;
  readonly #mediaProbe: MediaProbePort;
  readonly #options: MusicPlaybackClientOptions;
  readonly #allowedRawMediaHosts: ReadonlySet<string>;
  #runtime: MusicPlaybackRuntime | null = null;
  #startOperation: Promise<MusicPlaybackClientHealth> | null = null;
  #stopOperation: Promise<void> | null = null;
  #startupController: AbortController | null = null;
  #lifecycleState: MusicPlaybackClientState = "stopped";
  ...
  >();
  readonly #providerFailureCounts = new Map<MusicProviderKey, number>();
  #currentHealth: MusicPlaybackClientHealth = stoppedHealth();

  public constructor(
    factory: MusicPlaybackRuntimeFactory,
    ...
    this.#allowedRawMediaHosts = new Set(
      this.#options.allowedRawMediaHosts
    );
  }

  public health(): MusicPlaybackClientHealth {
    return this.#currentHealth;
  }

  public async start(
    signal?: AbortSignal
  ): Promise<MusicPlaybackClientHealth> {
    if (signal?.aborted === true) {
      throw abortError(
        new MusicPlaybackError(
          "MUSIC.MEDIA_ENGINE_UNAVAILABLE",
          "Music startup was cancelled.",
          true
        ),
        signal
      );
    ...
    if (this.#startOperation !== null) {
      if (signal === undefined) return this.#startOperation;
      return executeBounded(
        async () => await this.#startOperation!,
        120_000,
        () =>
          new MusicPlaybackError(
            "MUSIC.MEDIA_ENGINE_UNAVAILABLE",
            "The music client did not start in time.",
            true
          ),
        signal
      );
    ...
    if (this.#runtime !== null) {
      if (this.#currentHealth.state === "stopped") {
        throw new MusicPlaybackError(
          "MUSIC.MEDIA_ENGINE_UNAVAILABLE",
          "The previous music runtime shutdown is incomplete.",
          true
        );
      }
      return this.#currentHealth;
  ...
  async #startInternal(
    ...
    let mediaEngine: MediaEngineProbeResult;
    try {
      mediaEngine = normalizeProbeResult(
        await executeBounded(
          ...
          this.#options.mediaProbeTimeoutMs,
          () =>
            new MusicPlaybackError(
              "MUSIC.MEDIA_ENGINE_UNAVAILABLE",
              "The media engine probe timed out.",
              true
            ),
          startupSignal
        )
    ...
    if (mediaEngine.state !== "ready") {
      this.#currentHealth = Object.freeze({
        ...
        mediaEngine,
        providers: Object.freeze(
          MUSIC_PROVIDER_KEYS.map((provider) =>
            healthFor(
              provider,
              this.#options.enabledProviders.includes(provider)
                ? "unavailable"
                : "disabled",
              this.#options.enabledProviders.includes(provider)
    ...
    let runtime: MusicPlaybackRuntime;
    try {
      const runtimeOptions: MusicPlaybackRuntimeOptions = Object.freeze({
        ...
        maximumResolvedReferences:
          this.#options.maximumResolvedReferences,
        maximumPendingOperationsPerGuild:
    ...
    const providers: MusicProviderHealth[] = [];
    for (const provider of MUSIC_PROVIDER_KEYS) {
      ...
      try {
        await executeBounded(
          (signal) => runtime.registerProvider(provider, signal),
    ...
    const hasStreamingProvider = providers.some(
      (provider) =>
    ...
    const hasReadyBridge = providers.some(
      (provider) =>
        provider.state === "ready" &&
    ...
    if (!hasReadyBridge) {
      for (let index = 0; index < providers.length; index += 1) {
        ...
        if (
          ...
          providers[index] = healthFor(
            provider.provider,
            "unavailable",
            "MUSIC.METADATA_BRIDGE_UNAVAILABLE"
          );
        }
      }
    }
  ...
  async #stopInternal(): Promise<void> {
    const stopReason = clientNotReadyError(
      "The music playback client is stopping."
    );
  ...
  #detachRuntimeOperations(runtime: MusicPlaybackRuntime): void {
    for (const [controller, entry] of this.#inFlightOperations) {
      if (entry.runtime === runtime) this.#inFlightOperations.delete(controller);
    }
  }
    ...
    try {
      await executeBounded(
        ...
        () =>
          new MusicPlaybackError(
            "MUSIC.MEDIA_ENGINE_UNAVAILABLE",
            "Music operations did not drain in time.",
            true
          )
      );
    } catch {
  ...
  async #destroyRuntime(runtime: MusicPlaybackRuntime): Promise<void> {
    await executeBounded(
      (signal) => runtime.destroy(signal),
      this.#options.shutdownTimeoutMs,
      () =>
        new MusicPlaybackError(
          "MUSIC.MEDIA_ENGINE_UNAVAILABLE",
          "The music runtime did not stop in time.",
          true
        )
    );
  }

  #readyRuntime(): MusicPlaybackRuntime {
    if (
      ...
      throw new MusicPlaybackError(
        ...
        "The music playback client is not ready.",
        true
      );
    }
    return this.#runtime;
  ...
  #executeTrackedOperation<T>(
    ...
    if (
      ...
      return Promise.reject(
        new MusicPlaybackError(
          ...
          "The music operation capacity is temporarily exhausted.",
          true
        )
      );
    }
    ...
    this.#inFlightOperations.set(
      controller,
      Object.freeze({ guildId, runtime, operation: null })
    );
    const operationSignal =
      callerSignal === undefined
        ? controller.signal
        : AbortSignal.any([controller.signal, callerSignal]);
    const boundedOperation = executeBounded(
      (signal) => {
        ...
        this.#inFlightOperations.set(
          controller,
          Object.freeze({ guildId, runtime, operation: providerOperation })
        );
        void providerOperation.then(
          () => {
            if (
              this.#inFlightOperations.get(controller)?.operation ===
              providerOperation
            ) {
  ...
  #readyProviders(): ReadonlySet<MusicProviderKey> {
    return new Set(
      this.#currentHealth.providers
        .filter((provider) => provider.state === "ready")
        .map((provider) => provider.provider)
    );
  }

  #recordProviderFailure(provider: MusicProviderKey): void {
    ...
    const providers = this.#currentHealth.providers.map((entry) =>
      entry.provider === provider && entry.state === "ready"
        ? healthFor(
            provider,
            "unavailable",
    ...
    const hasReadyStreamingProvider = providers.some(
      (entry) =>
        entry.state === "ready" &&
        (MUSIC_STREAM_PROVIDER_KEYS as readonly MusicProviderKey[]).includes(
          entry.provider
  ...
  #recordProviderSuccess(provider: MusicProviderKey): void {
    this.#providerFailureCounts.delete(provider);
  }

  #normalizeCandidate(
    candidate: MusicRuntimeCandidate,
    ...
    if (
      ...
      throw new MusicPlaybackError(
        "MUSIC.PROVIDER_UNAVAILABLE",
        "The music runtime returned an invalid provider result.",
        true
      );
    }
  ...
  public async resolve(input: {
    ...
    const providerPolicies = normalizeMusicProviderPolicies(
    ...
    const maximumResults = Math.min(
      input.maximumResults ?? this.#options.maximumSearchResults,
      this.#options.maximumSearchResults
    );
    if (!Number.isSafeInteger(maximumResults) || maximumResults <= 0) {
      throw new MusicPlaybackError(
        "MUSIC.MEDIA_QUERY_INVALID",
        "The media result limit is invalid.",
        false
      );
    }
    ...
    const allowedBridgeProviders = this.#options.bridgeProviderOrder.filter(
      (provider) =>
        enabledProviders.has(provider) && readyProviders.has(provider)
    );
    if (
      (selection.provider === "spotify" ||
        selection.provider === "apple_music") &&
      allowedBridgeProviders.length === 0
    ) {
      throw new MusicPlaybackError(
        "MUSIC.PROVIDER_UNAVAILABLE",
        "No enabled streaming provider can bridge this metadata source.",
        true
      );
    }
    let resolved: readonly MusicRuntimeCandidate[];
    try {
      resolved = await this.#executeTrackedOperation(
        runtime,
        guildId,
        (activeRuntime, signal) =>
        ...
        this.#options.mediaResolutionTimeoutMs,
        () =>
          new MusicPlaybackError(
            "MUSIC.MEDIA_RESOLUTION_TIMEOUT",
            "The media provider did not answer in time.",
            true
          ),
        input.signal
      );
      ...
      if (error instanceof MusicPlaybackError) throw error;
      throw new MusicPlaybackError(
        "MUSIC.PROVIDER_UNAVAILABLE",
        "The selected media provider is temporarily unavailable.",
        true
      );
    }
    ...
    if (
      ...
      throw new MusicPlaybackError(
        ...
        resolvedLength === 0
          ...
          : "The music runtime returned an invalid result collection.",
        resolvedLength !== 0
      );
    }
    ...
    for (let index = 0; index < resolved.length; index += 1) {
      ...
      if (descriptor === undefined || !("value" in descriptor)) {
        throw new MusicPlaybackError(
          "MUSIC.PROVIDER_UNAVAILABLE",
          "The music runtime returned a sparse result collection.",
          true
        );
      }
      items.push(
        this.#normalizeCandidate(
          descriptor.value as MusicRuntimeCandidate,
          selection.provider
        )
      );
    }
  ...
  #normalizeSnapshot(
    snapshot: MusicSessionSnapshot | null,
    guildId: string,
    voiceChannelId?: string
  ): MusicSessionSnapshot | null {
    ...
    if (
      ...
      !["connecting", "playing", "paused", "idle"].includes(
        snapshot.state
      ) ||
      ...
      throw new MusicPlaybackError(
        "MUSIC.MEDIA_ENGINE_UNAVAILABLE",
        "The music runtime returned an invalid session snapshot.",
        true
      );
    }
    const current: unknown = snapshot.current;
    if (
      current !== null &&
      (typeof current !== "object" ||
        !isMusicProviderKey(
          (current as { readonly sourceProvider?: unknown })
            .sourceProvider
        ))
    ) {
      throw new MusicPlaybackError(
        "MUSIC.MEDIA_ENGINE_UNAVAILABLE",
        "The music runtime returned an invalid current track.",
        true
      );
    }
    const typedCurrent = current as MusicRuntimeCandidate | null;
    const normalizedCurrent =
      typedCurrent === null
        ? null
        : this.#normalizeCandidate(
            typedCurrent,
            typedCurrent.sourceProvider
          );
    return Object.freeze({
      guildId,
      voiceChannelId: boundedIdentity(
  ...
  public async enqueue(input: {
    ...
    const runtime = this.#readyRuntime();
    const guildId = boundedIdentity(input.guildId, "Guild id");
    const voiceChannelId = boundedIdentity(
    ...
    const providerReference = boundedReference(
      input.media.providerReference,
      "provider reference"
    );
    try {
      const snapshot = await this.#executeTrackedOperation(
        runtime,
        guildId,
        (activeRuntime, signal) =>
          activeRuntime.enqueue({
            guildId,
            voiceChannelId,
            providerReference,
      ...
      if (normalized === null) {
        throw new MusicPlaybackError(
          "MUSIC.MEDIA_ENGINE_UNAVAILABLE",
          "The runtime did not create a music session.",
          true
        );
      }
      return normalized;
      ...
      throw new MusicPlaybackError(
        "MUSIC.MEDIA_ENGINE_UNAVAILABLE",
        "The audio playback engine is temporarily unavailable.",
        true
      );
    }
  }
    ...
    try {
      const snapshot = await this.#executeTrackedOperation(
        ...
        () =>
          new MusicPlaybackError(
            "MUSIC.PLAYBACK_TIMEOUT",
            "The playback control timed out.",
            true
          ),
        input.signal
      );
      ...
      throw new MusicPlaybackError(
        "MUSIC.MEDIA_ENGINE_UNAVAILABLE",
        "The playback control could not be applied.",
        true
      );
    }
  }
  ...
  public async session(
    ...
    const snapshot = await this.#executeTrackedOperation(
      runtime,
      guildId,
      (activeRuntime, operationSignal) =>
        activeRuntime.session(guildId, operationSignal),
      this.#options.sessionReadTimeoutMs,
      () =>
        new MusicPlaybackError(
          "MUSIC.MEDIA_ENGINE_UNAVAILABLE",
          "The music session read timed out.",
          true
        ),
      signal
    );
